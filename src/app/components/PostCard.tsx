...
export default function PostCard({ title, slug, date, category }: PostCardProps) {
  ...
  return (
    <article className="group transition-all duration-300 flex flex-col bg-white rounded-lg shadow-sm hover:shadow-md overflow-hidden border border-gray-100">
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-[var(--primary-light)]/10 text-[var(--primary)]">{category}</span>
        ...
        <Link href={`/blog/${slug}`} className="block group-hover:text-[var(--primary)] transition-colors duration-200">
          <h3 className="text-xl md:text-2xl font-semibold mb-3" style={{ fontFamily: "var(--font-cormorant)" }}>
            {title}
          </h3>
        </Link>
